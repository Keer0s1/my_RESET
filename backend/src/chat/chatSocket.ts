    ...
    socket.on(
      ...
      (payload: ChatJoinPayload, callback: (ack: ChatJoinAck) => void) => {
        try {
          ...
          callback({ ok: false, error: e instanceof Error ? e.message : 'Ошибка обработки join' });
        }
      }
    );
    ...
    socket.on(
      ...
      (payload: ChatSendPayload, callback: (ack: ChatSendAck) => void) => {
        try {
          ...
          callback({ ok: false, error: e instanceof Error ? e.message : 'Ошибка отправки сообщения' });
        }
      }
    );
